 ...
 */
public class NaiveBayesMultinomialAggressive extends AbstractClassifier {
    ...
    protected double[][] lastObservedTime;
    protected double[] lastObservedClassTime;

    private double decayDegree = 0.0;
    ...
    protected DoubleVector[] m_faded_wordTotalForClass;
    ...
    public void trainOnInstanceImpl(Instance inst) {
        if (this.reset == true) {
            ...
            m_faded_wordTotalForClass = new DoubleVector[m_numClasses];
            for (int i = 0; i< m_numClasses; i++) {
                //Arrays.fill(wordTotal, laplace);
                m_faded_wordTotalForClass[i] = new DoubleVector();
            }

            lastObservedTime = new double[numAttributes][m_numClasses];
            lastObservedClassTime = new double[m_numClasses];
            for (double[] times : lastObservedTime) {
                Arrays.fill(times, 0.0);
            }
            for (double time : lastObservedClassTime) {
                time = 0.0;
            }
            this.reset = false;
        }
        ...
        for (int i = 0; i < inst.numValues(); i++) {
            ...
            if (index != classIndex && !inst.isMissing(i)) {
                m_wordTotalForClass[classValue].setValue(index, m_faded_wordTotalForClass[classValue].getValue(index));
                //m_wordTotalForClass[index][classValue] += w * inst.valueSparse(i);
                if(timestamp >= lastObservedTime[index][classValue])
                    lastObservedTime[index][classValue] = timestamp;
    ...
    public double[] getVotesForInstance(Instance instance) {
        ...
        for (int i = 0; i < m_numClasses; i++) {
            ...
            if(instance.weight() >= lastObservedClassTime[i])
                classInterval = instance.weight() - lastObservedClassTime[i];
            double decayFactor = decay(classInterval);
            m_probOfClass[i] = m_probOfClass[i] * decayFactor;
            //double fadedClassProb = m_probOfClass[i] * decayFactor;
            probOfClassGivenDoc[i] = Math.log(m_probOfClass[i]) - totalSize * Math.log(m_classTotals[i]);
            //probOfClassGivenDoc[i] = Math.log(fadedClassProb) - totalSize * Math.log(m_classTotals[i]);
        }
        ...
        for (int i = 0; i < instance.numValues(); i++) {
            ...
            for (int c = 0; c < m_numClasses; c++) {
                double timestamp = instance.weight(); //timestamp, naming is confusing
                double lastTime = lastObservedTime[index][c];
                double timeInterval = 0.0;
                if(timestamp >= lastTime)
                    timeInterval = timestamp - lastTime;

                double decayed = decay(timeInterval);
                //double value = decayed * m_wordTotalForClass[c].getValue(index);
                //m_wordTotalForClass[c].setValue(index, decayed * m_wordTotalForClass[c].getValue(index));
                m_faded_wordTotalForClass[c].setValue(index, decayed * m_wordTotalForClass[c].getValue(index));
                double value = m_wordTotalForClass[c].getValue(index);
                if(value <  0.1 && value != 0.0) {
                    smallestProb = value;
                    //System.out.println("Logarithmic Value: " + Math.log(value) + "Value: " + value);
                }
        ...
        double[] returnValue = Utils.logs2probs(probOfClassGivenDoc);
        //System.out.println("Prob 0: " + returnValue[0] + " Prob 1: " +  returnValue[1]);
